...
def train_model(model, X_train, y_train, optimizer, criterion):
	...
	chunk_loss = 0 
	target = Variable(torch.ones(batch_size), requires_grad=True)
	...
	num_chunks = int(X_train.shape[0] / batch_size)
	return chunk_loss / num_chunks


#initialize the model 
model_PATH = "model1.pt"
model = ShallowNet()
criterion = nn.CosineEmbeddingLoss()  #mean squared error
